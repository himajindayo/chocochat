...
function registerHandlers(socket, io, db, clientIp) {
  ...
  socket.on('accountLogin', async (data, cb) => {
    ...
    const userId = typeof data?.userId === 'string' ? data.userId.trim() : '';
    const uidErr = validate.validateUserId(userId);
  ...
  socket.on('sendMessage', async (data, cb) => {
    ...
    if (message.startsWith('/')) {
      ...
      if (result.type === 'broadcast_message') {
        const msg = buildMsg(currentUserId, currentAccount.username, result.message, color, replyTo, isAdmin, currentAccount.statusText || '');
  ...
  socket.on('editMessage', async (data, cb) => {
    ...
    const messageErr = validate.validateMessage(data?.message);
