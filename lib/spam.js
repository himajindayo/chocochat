...
const profileUpdateTimes = new Map();
const PROFILE_UPDATE_WINDOW = 5 * 60_000;
const PROFILE_UPDATE_LIMIT = 6;
const PROFILE_UPDATE_LOCKOUT = 10 * 60_000;
const profileUpdateLockouts = new Map();
function isProfileUpdateRateLimited(userId) {
    const now = Date.now();
    const lock = profileUpdateLockouts.get(userId);
    if (lock && now < lock)
        return { limited: true, remaining: Math.ceil((lock - now) / 1000) };
    return { limited: false };
}
function recordProfileUpdate(userId) {
    const now = Date.now();
    const hist = (profileUpdateTimes.get(userId) || []).filter(t => now - t < PROFILE_UPDATE_WINDOW);
    hist.push(now);
    profileUpdateTimes.set(userId, hist);
    if (hist.length >= PROFILE_UPDATE_LIMIT) {
        profileUpdateLockouts.set(userId, now + PROFILE_UPDATE_LOCKOUT);
        profileUpdateTimes.delete(userId);
    }
}
...
function clearUserData(userId) {
    ...
    profileUpdateTimes.delete(userId);
    profileUpdateLockouts.delete(userId);
}
const cleanupTimer = setInterval(() => {
    ...
    for (const [uid, ts] of profileUpdateLockouts) {
        if (now >= ts) {
            profileUpdateLockouts.delete(uid);
            profileUpdateTimes.delete(uid);
        }
    }
    for (const [uid, hist] of profileUpdateTimes) {
        const active = hist.filter(t => now - t < PROFILE_UPDATE_WINDOW);
        if (active.length === 0)
            profileUpdateTimes.delete(uid);
        else
            profileUpdateTimes.set(uid, active);
    }
}, 10 * 60_000);
cleanupTimer.unref();
module.exports = {
