...
const { USER_ID_RE, COLOR_RE, SUPER_ADMIN_ID, UUID_RE } = require('./constants');

function validateUserId(userId, options = {}) {
  ...
  if (!allowSuperAdmin && userId.toUpperCase() === SUPER_ADMIN_ID) return 'このユーザーIDは使用できません';
  return null;
}
...
function validateUsername(username) {
  ...
  if (value.length === 0) return 'ユーザー名を入力してください';
  if (value.length > 20)   return 'ユーザー名は20文字以内で入力してください';
  if (value.includes('管理者')) return 'ユーザー名に「管理者」は含められません';
  return null;
}
...
function validateStatusText(text) {
  ...
  if (text.trim().length > 30) return 'ステータスは30文字以内で入力してください';
  return null;
}
...
function validateMessageId(id) {
  if (typeof id !== 'string' || !UUID_RE.test(id)) return 'メッセージIDが不正です';
  return null;
}

module.exports = {
  validateUserId, validateUsername, validatePassword,
  validateMessage, validateStatusText, validateColor, validateMessageId,
};
