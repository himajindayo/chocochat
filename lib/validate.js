'use strict';
const { USER_ID_RE, COLOR_RE, SUPER_ADMIN_ID, UUID_RE, PASSWORD_MIN_LENGTH, PASSWORD_MAX_LENGTH } = require('./constants');
function validateUserId(userId, options = {}) {
    const allowSuperAdmin = options.allowSuperAdmin === true;
    if (typeof userId !== 'string' || userId.length === 0)
        return 'ユーザーIDを入力してください';
    if (userId.length < 4 || userId.length > 20)
        return 'ユーザーIDは4〜20文字で入力してください';
    if (!USER_ID_RE.test(userId))
        return 'ユーザーIDは半角英数字のみ使用できます';
    if (!allowSuperAdmin && userId.toUpperCase() === SUPER_ADMIN_ID)
...
function validateUsername(username) {
    ...
    if (value.includes('管理者'))
        return 'ユーザー名に「管理者」は含められません';
    return null;
}
