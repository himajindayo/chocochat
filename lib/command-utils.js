...
function forEachSocket(session, userId, callback) {
  for (const sid of session.getSocketIds(userId)) {
    const socket = callback ? callback(sid) : null;
    if (socket === false) break;
  }
}

function emitToUserSockets(io, session, userId, event, payload) {
  forEachSocket(session, userId, sid => io.sockets.sockets.get(sid)?.emit(event, payload));
}

function emitToUserSocketsWithSocket(io, session, userId, callback) {
  forEachSocket(session, userId, sid => {
