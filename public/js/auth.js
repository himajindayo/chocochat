...
function switchForm(f) {
    ...
    if (f === 'signup')
        setTimeout(() => byId('s-uid')?.focus(), 0);
}
onClick('to-signup', () => switchForm('signup'));
onClick('to-login', () => switchForm('login'));
setTimeout(() => byId('l-uid')?.focus(), 0);
function doLogin() {
    const userId = byId('l-uid').value.trim();
    const password = byId('l-pass').value;
    if (!userId)
        return setAuthMsg('ユーザーIDを入力してください');
    if (!password)
        return setAuthMsg('パスワードを入力してください');
    socket.emit('accountLogin', { userId, password }, onLoginResp);
}
onClick('login-btn', doLogin);
onKeydown('l-uid', e => { if (e.key === 'Enter')
    doLogin(); });
onKeydown('l-pass', e => { if (e.key === 'Enter')
    doLogin(); });
onClick('signup-btn', () => {
    const userId = byId('s-uid').value.trim();
    const username = byId('s-uname').value.trim();
    const password = byId('s-pass').value;
    const confirmPassword = byId('s-pass2').value;
    if (!/^[A-Za-z0-9]{4,20}$/.test(userId))
        return setAuthMsg('ユーザーIDは半角英数字4〜20文字で入力してください');
