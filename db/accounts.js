...
async function loginWithToken(token) {
    const res = await pool.query('SELECT * FROM accounts WHERE login_token = $1', [sha256Hex(token)]);
    if (res.rows.length === 0) {
...
async function updateProfile(userId, { color, theme, statusText, username } = {}) {
    ...
    if (setClauses.length === 0)
        return { success: true };
    vals.push(userId);
    const res = await pool.query(`UPDATE accounts SET ${setClauses.join(', ')} WHERE user_id = $${i} RETURNING *`, vals);
    if (res.rows.length === 0) {
        return { success: false, error: 'アカウントが見つかりません' };
    }
