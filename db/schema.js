...
async function createTables(pool) {
  ...
  `);
    ...
    ADD COLUMN IF NOT EXISTS created_ip_hash CHAR(64) NOT NULL DEFAULT ''
  `);
    await pool.query(`
    CREATE INDEX IF NOT EXISTS idx_accounts_created_ip_hash
    ON accounts(created_ip_hash)
  `);
    await pool.query(`
    DROP TABLE IF EXISTS registration_ip_accounts
  `);
    await pool.query(`
  ...
  `);
    await pool.query(`
    CREATE INDEX IF NOT EXISTS idx_public_messages_ts
  ...
  `);
    await pool.query(`
    CREATE INDEX IF NOT EXISTS idx_moderation_actions_ip
    ON moderation_actions(target_ip_hash)
    WHERE action = 'ban' AND target_ip_hash IS NOT NULL
  `);
    await pool.query(`
    CREATE INDEX IF NOT EXISTS idx_moderation_actions_expiry
